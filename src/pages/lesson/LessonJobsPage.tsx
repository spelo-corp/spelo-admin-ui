        ...
        async (page: number, isRefresh: boolean = false) => {
            ...
            try {
                ...
                const [allJobsRes, audioRes] = await Promise.all([
                    ...
                    api
                        .getAudioProcessingJobs({ lessonId: numericLessonId, page, size: pageSize })
                        .catch(() => ({
                            content: [] as AudioJob[],
                            pageNumber: 1,
                            pageSize: 20,
                ...
                if (page === 1 && allJobsRes.success && allJobsRes.data) {
                    const convertedJobs: (ProcessingJob & { job_type?: string })[] =
                        allJobsRes.data.map((job: any) => ({
                            id: job.id,
                ...
                {loading ? (
                    ...
                    <>
                                            ...
                                            >
                                                <td className="px-5 py-3">
                                                    ...
                                                    {isProcessing && (
                                                        <div className="text-[11px] text-slate-500">
                                                            ...
                                                            ).original_audio_url?.slice(0, 30) ||
                                                                "—"}
                                                            {(job as ProcessingJob)
                                                                .original_audio_url?.length > 30
                                                                ? "…"
                                                                : ""}
                                                        </div>
                                                    )}
                                                    {!isProcessing && (
                                                        <div className="text-[11px] text-slate-500">
                                                            {(job as AudioJob).transcript?.slice(
                                                                0,
                                                                30,
                                                            ) || "—"}
                                                            {(job as AudioJob).transcript?.length >
                                                            30
                                                                ? "…"
                                                                : ""}
                                                        </div>
                                                ...
                                                <td className="px-5 py-3">
                                                    {isProcessing ? (
                                                        <JobStatusBadge
                                                            status={
                                                                (job as ProcessingJob).current_step
                                                            }
                                                        />
                                                    ) : (
                                                        <StatusBadge
                                                            status={(job as AudioJob).status}
                                                        />
                                                    )}
                                                </td>
                        ...
                        {hasMore && (
                            <div
                                ref={observerTarget}
                                className="px-5 py-4 border-t border-slate-100 bg-slate-50"
                            >
                                {loadingMore ? (
